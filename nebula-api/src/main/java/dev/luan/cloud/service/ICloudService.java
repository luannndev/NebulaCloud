...
import dev.luan.cloud.networking.packet.service.PacketPlayOutCopyServiceToTemplate;
...
public interface ICloudService extends Serializable {
    ...
    default void copy(ITemplate template) {
        NebulaAPI.getNebulaAPI().getProvider().getNebulaNetworking().sendPacket(new PacketPlayOutCopyServiceToTemplate(getServiceName(), template.getPath().toString()));
    }
