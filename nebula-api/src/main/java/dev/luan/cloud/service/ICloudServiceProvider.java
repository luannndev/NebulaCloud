...
public interface ICloudServiceProvider {
    ...
    default long gloablUsedMemory() {
        ...
        NebulaAPI.getNebulaAPI().getProvider().getServiceProvider().getAllServices().stream().filter(cloudService -> cloudService.getServiceNode().getName().equals(NebulaAPI.getNebulaAPI().getProvider().getThisNode().getName())).forEach(cloudService -> globalUsedMemory.getAndAdd(cloudService.getMaxMemory()));
